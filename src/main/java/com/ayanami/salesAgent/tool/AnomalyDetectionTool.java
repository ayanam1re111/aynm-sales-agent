...
public class AnomalyDetectionTool {
    ...
    public String detectAllAnomalies() {
      ...
      //执行4个检测方法，收集所有异常，按优先级排序，拼接成一段文本返回用户
        try {
            anomalies.addAll(detectRegionDropAnomalies());//地区销售额下降异常
            anomalies.addAll(detectZeroSaleProducts());//商品零销售异常
            anomalies.addAll(detectHighRefundReps());//高退单率员工异常
            anomalies.addAll(detectRepPerformanceDrop());//员工业绩下滑异常
        } catch (Exception e) {
            log.error("异常检测出错", e);
